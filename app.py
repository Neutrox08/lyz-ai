...
def inject_pwa_and_analytics():
  try:
    ...
    if index_path.exists():
      ...
      # Snippet combinado de Google Analytics y PWA Manifest/Iconos
      pwa_ga_snippet = """
            <!-- Google tag (gtag.js) -->
      ...
      if "G-SE3F0436R" not in content or "manifest.json" not in content:
        # Si ya tiene una parte, limpiamos o reemplazamos con seguridad antes de </head>
...
if st.session_state.current_view == "modifier":
  ...
  for i in range(4):
    with cols[i]:
      genre_name = favs[i]
      if st.button(f"📖 {genre_name}", use_container_width=True):
        selected_prompt = (
...
all_genres = [
    ...
    "Fanfic", "Poesía", "Ensayo / Filosofía", "Realismo Mágico", "Distopía"
]
...
if "current_work_genre" not in st.session_state:
    st.session_state.current_work_genre = None
if "favorite_shortcuts" not in st.session_state:
    st.session_state.favorite_shortcuts = ["Fantasía", "Ciencia Ficción", "Romance", "Misterio y Thriller"]
if "current_view" not in st.session_state:
    st.session_state.current_view = "chat"
...
if st.session_state.current_view == "modifier":
    ...
    if not works_dict:
        ...
        for title, content in list(works_dict.items()):
            with st.container(border=True):
                st.subheader(f"📖 {title}")
                st.text_area("Contenido:", value=content[:300] + "...", height=100, disabled=True, key=f"prev_{genre}_{title}")
                col1, col2 = st.columns(2)
                with col1:
                    st.download_button("📥 Descargar TXT", data=content, file_name=f"{title}.txt", mime="text/plain", key=f"dl_{genre}_{title}")
                with col2:
                    if st.button("🗑️ Eliminar", key=f"del_{genre}_{title}"):
